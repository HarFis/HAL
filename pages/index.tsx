...
export default function Home() {
  ...
  const onFinish = async ({ link }: FormValues) => {
    try {
      ...
      setStatus('error');
      setMessage(error.response?.data?.error_description || 'Something went wrong, idiot!');
    }
  }
  ...
  return (
    <Layout>
      <Head>
        <title>Hardy's Link Shortner</title>
            ...
            <div className={styles.linkField}>
              <div className={styles.linkFieldInput}>
                <Form.Item name="link" noStyle rules={[{
                  ...
                  <Input placeholder="https://my-super-long-link.com/that-dont-like-to-see-anymore" size="large"/>
                </Form.Item>
              </div>
      ...
      <Footer className={styles.footer}>
        Hardy's Link Shortener (HLS) &copy; 2021
      </Footer>
    </Layout>
  )
}